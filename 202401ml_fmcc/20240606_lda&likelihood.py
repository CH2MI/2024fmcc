...
def wav2mfcc(type, cnt, mfcc_length, n_mfcc):
    mfccs = []
    with open('fmcc_' + type + '.ctl', 'r') as f:
        for i in range (cnt):
            path = f.readline().strip()
            path = 'raw16k/' + type + '/' + path + '.wav'
            mfcc = MFCC(path, n_mfcc).T
            mfccs.append(mfcc)
...
for index in range (10, 101):
    mfccs, mfcc_length = wav2mfcc('train', 8000, mfcc_length, index)
    print('mfcc 추출 완료')
    ...
    train_p.append(accuracy * 100)

    #####################################################################################

    mfccs, mfcc_length = wav2mfcc('test', 1000, mfcc_length, index)
    print('mfcc 추출 완료')
...
with open('n_mfcc_train.txt', 'w') as f:
    for i in range(90):
        f.write(f'{i} {train_p[i]}\n')

with open('n_mfcc_text.txt', 'w') as f:
    for i in range(90):
        f.write(f'{i} {test_p[i]}\n')